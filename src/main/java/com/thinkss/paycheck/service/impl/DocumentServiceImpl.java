...
public class DocumentServiceImpl implements DocumentService {
	...
	public List<DocumentSubType> findByDocumentType(DocumentType documentType) {
		Query query = em.createQuery("select dst from DocumentSubType dst where  documentType = :documentType ")
	...
	public DocumentDetails findDocumentDetailByUser(User user, DocumentType documentTypes) {
		Query query = em
				.createQuery("select di from DocumentDetails di where user = :user and dcumentType = :dcumentType "
				).setParameter("user", user).setParameter("dcumentType", documentTypes);
